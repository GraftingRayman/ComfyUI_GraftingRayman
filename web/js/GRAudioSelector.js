...
console.log("GRAudioSelector extension loading...");

// Register the custom node
app.registerExtension({
    name: "GRAudioSelector",
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "GRAudioSelector") {
                ...
                if (type === 1) { // 1 = input, 2 = output
                    if (connected) {
                        ...
                        if (allConnected) {
                            ...
                            this.addInput(`audio_${newIndex}`, "AUDIO");
                        }
                    } else {
    ...
    async nodeCreated(node) {
        if (node.comfyClass === "GRAudioSelector") {
