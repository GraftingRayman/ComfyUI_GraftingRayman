...
def apply_shape_transform(sigmas, shape_id):
    """Apply the same shape transformations as in the JavaScript frontend"""
    if shape_id == "preset" or shape_id is None:
        return sigmas
    ...
    # Define shape functions matching CURVE_SHAPES in JavaScript
    shape_fns = {
        "linear": lambda t: t,
        "easein": lambda t: t * t,
        "easeout": lambda t: 1 - (1 - t) * (1 - t),
        "cosine": lambda t: (1 - np.cos(t * np.pi)) / 2,
        "s-curve": lambda t: 2 * t * t if t < 0.5 else 1 - 2 * (1 - t) * (1 - t),
    }
    
    if shape_id not in shape_fns:
        return sigmas
    
    fn = shape_fns[shape_id]
    t_values = np.linspace(0, 1, n)
    
    # Transform while preserving endpoints
    transformed = np.array([start + (end - start) * fn(t) for t in t_values])
    
    # Ensure monotonic decreasing (last value should be end)
    transformed[-1] = end
...
class GRSigmaPresetSelectorAdvanced:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            ...
            "hidden": {
                "shape": ("STRING", {"default": "preset"}),  # Hidden widget to sync with frontend
            }
        }
    ...
    def get_sigmas(self, preset, steps, auto_scale_to_steps, shape="preset"):
        # Generate raw sigmas from preset
        raw_sigmas = get_sigmas_for_preset(preset)
        
        # Apply shape transformation (syncs with frontend shape buttons)
        transformed_sigmas = apply_shape_transform(raw_sigmas, shape)
        
        sigma_tensor = torch.tensor(transformed_sigmas, dtype=torch.float32)
        # Generate graph from transformed sigmas so preview matches frontend
        graph_tensor = self._generate_graph(transformed_sigmas, preset)
