...
class GRCounterVideo:
    ...
    RETURN_NAMES = ("FileDetails", "Image")
...
class GRScrollerVideo:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "movement_type": (
                    ["scroll", "bounce", "wave", "jitter", "circular", "zoom", "fade", "rotate", "oscillate", "diagonal", "spiral", "flip", "random_walk"],
                    {"default": "scroll"},
                ),
    ...
    @staticmethod
    def generate_frames(text, font_path, font_size, font_color, font_opacity, background_colour, use_background_image, background_image, resolution, scroll_direction, scroll_speed, movement_speed, movement_duration, fps, movement_type, movement_distance, apply_to_each_character, seed, font_stroke, font_stroke_thickness, font_stroke_colour):
        frames = []
        ...
        if use_background_image and background_image is not None:
            if isinstance(background_image, torch.Tensor):
                if background_image.dim() == 4:
                    background_image = background_image[0].cpu().numpy()  # Use the first image in the batch and convert to numpy
        ...
        if apply_to_each_character:
            movement_types = ["bounce", "wave", "jitter", "circular", "zoom", "fade", "rotate", "oscillate", "diagonal", "spiral", "flip", "random_walk"]
        ...
        for frame_index in range(total_frames):
            ...
            if movement_type == "scroll":
                ...
                GRScrollerVideo.draw_text(draw, (x_position, y_position), text, font, font_color, font_stroke, font_stroke_thickness, font_stroke_colour)
                ...
                for i, char in enumerate(text):
                    ...
                    if char_movement_type == "bounce":
                        ...
                        GRScrollerVideo.draw_text(draw, (char_x + offset_x, char_y + offset_y), char, font, temp_font_color, font_stroke, font_stroke_thickness, font_stroke_colour)
                    ...
                    if apply_to_each_character:
                        if char_image:
                            frame.paste(char_image, (char_x + offset_x, char_y + offset_y), char_image)
                        else:
                            GRScrollerVideo.draw_text(draw, (char_x + offset_x, char_y + offset_y), char, font, font_color, font_stroke, font_stroke_thickness, font_stroke_colour)
                    else:
            ...
            frames.append(np.array(frame))  # Convert PIL image to numpy array before adding to the list
            pbar.update_absolute(frame_index)  # Update the progress bar

        return frames

    @staticmethod
    def get_file_details(file_path, text, font_path, font_size, font_color, font_opacity, background_colour, use_background_image, resolution, scroll_direction, scroll_speed, movement_speed, movement_duration, fps, movement_type, movement_distance, apply_to_each_character, seed, font_stroke, font_stroke_thickness, font_stroke_colour):
        file_size = os.path.getsize(file_path)
        file_details = (
            f"File Path: {file_path}, File Size: {file_size} bytes\n"
    ...
    @classmethod
    def generate_video(cls, text, font_path, font_size, font_color, font_opacity, background_colour, use_background_image=False, background_image=None, resolution='HD 1280x720', scroll_direction='left', scroll_speed=1.0, movement_speed=1.0, movement_duration=10, output_path='H:\\scroller\\output_video.mp4', save_images=False, image_save_path='H:\\scroller\\frames\\', fps=30, movement_type='scroll', movement_distance=20, apply_to_each_character=False, seed=42, font_stroke=False, font_stroke_thickness=2, font_stroke_colour="black", upscale=1.0):
        # Append date and time to the output path
        now = datetime.now().strftime("%d-%y-%m-%H-%M")
        output_path = os.path.splitext(output_path)[0] + f"_{now}.mp4"

        frames = cls.generate_frames(text, font_path, font_size, font_color, font_opacity, background_colour, use_background_image, background_image, resolution, scroll_direction, scroll_speed, movement_speed, movement_duration, fps, movement_type, movement_distance, apply_to_each_character, seed, font_stroke, font_stroke_thickness, font_stroke_colour)
        
        # Save video using the original RGBA frames
        output_file = save_video(frames, output_path, fps, background_colour)
        ...
        # Convert frames to tensors and stack them into a batch
        frame_tensors = [torch.from_numpy(frame).permute(0, 1, 2) for frame in frames]  # Convert to (C, H, W) format
        ...
        file_details = cls.get_file_details(output_file, text, font_path, font_size, font_color, font_opacity, background_colour, use_background_image, resolution, scroll_direction, scroll_speed, movement_speed, movement_duration, fps, movement_type, movement_distance, apply_to_each_character, seed, font_stroke, font_stroke_thickness, font_stroke_colour)
